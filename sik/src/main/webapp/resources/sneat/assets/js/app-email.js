...
document.addEventListener("DOMContentLoaded", function() {
    {
        ...
        const r = document.querySelector(".email-list")
          ...
          , w = [].slice.call(document.querySelectorAll(".email-list-item-actions li"));
        ...
        p && new Quill(".email-reply-editor",{
            modules: {
        ...
        v && v.forEach(e=>{
            e.addEventListener("click", e=>{
                ...
                e.stopPropagation(),
                l ? t.removeAttribute("data-starred") : t.setAttribute("data-starred", "true")
            }
            )
        }
        ),
        ...
        i && i.forEach(e=>{
            e.addEventListener("click", e=>{
                ...
                t < i.length ? 0 == t ? g.indeterminate = !1 : g.indeterminate = !0 : t == i.length ? (g.indeterminate = !1,
                g.checked = !0) : g.indeterminate = !1
            }
            )
        }
        ),
        h && h.addEventListener("keyup", e=>{
            ...
            (t = "inbox" != a ? 
				[].slice.call(document.querySelectorAll(".email-list-item[data-" + a + '="true"]')) : [].slice.call(document.querySelectorAll(".email-list-item"))).forEach(e=>{
        ...
        n.forEach(e=>{
            e.addEventListener("click", e=>{
                ...
                o.forEach(e=>{
                    "inbox" == l || e.hasAttribute("data-" + l) ? (e.classList.add("d-block"),
        ...
        k.addEventListener("hidden.bs.modal", e=>{
            document.querySelector(".email-editor .ql-editor #email-subject").innerHTML = "",
            $("#emailContacts").val(""),
            a()
        }
        ),
        ...
        f && f.addEventListener("click", e=>{
            i.forEach(e=>{
                ...
                e.parentNode.closest("li.email-list-item").classList.add("email-marked-read"),
                e = e.parentNode.closest("li.email-list-item").querySelector(".email-list-item-actions li"),
                Helpers._hasClass("email-read", e)) && (e.classList.remove("email-read"),
        ...
        l.length && l.on("click", function() {
            ...
            //e.text(e.text() == '열기' ? '닫기' : '열기' );
    
            //if(e.text() == '열기'){  
            //	e.parents().find(".email-card-last").removeClass("hide-pseudo")
			// }
        ...
        function a() {
            function e(e) {
                return e.id ? "<div class='d-flex flex-wrap align-items-center'><div class='avatar avatar-xs me-2'><img src='/resources/upload" + $(e.element).data("avatar") + "' alt='avatar' class='rounded-circle' /></div>" + e.text + "</div>" : e.text
            }
        ...
        w && w.forEach(e=>{
            e.addEventListener("click", e=>{
                e.stopPropagation();
                e = e.currentTarget;
                Helpers._hasClass("email-delete", e) ? e.parentNode.closest("li.email-list-item").remove() : Helpers._hasClass("email-read", e) ? (e.parentNode.closest("li.email-list-item").classList.add("email-marked-read"),
                Helpers._toggleClass(e, "email-read", "email-unread"),
                Helpers._toggleClass(e.querySelector("i"), "bx-envelope-open", "bx-envelope")) : Helpers._hasClass("email-unread", e) && (e.parentNode.closest("li.email-list-item").classList.remove("email-marked-read"),
                Helpers._toggleClass(e, "email-read", "email-unread"),
                Helpers._toggleClass(e.querySelector("i"), "bx-envelope-open", "bx-envelope"))
            }
            )
        }
        )
